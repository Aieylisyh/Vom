Shader "Curved/Ramp-Rim-Directional GrayScale" {
    Properties{
    ...
    SubShader{
        ...
        inline half4 LightingToonRamp(SurfaceOutput s, half3 lightDir, half atten)
        {
			...
			float color = c.r*.33 + c.g*.33 + c.b*.33;
            c.rgb = float3(color,color*.9,color*.9)*_GSF;
            c.a = 0;
            return c;
        }
        ...
        void surf(Input IN, inout SurfaceOutput o) {
			...
			float color = c.r*.33 + c.g*.33 + c.b*.33;
            o.Albedo = float3(color,color*.9,color*.9)*_GSF;
            o.Alpha = c.a;
