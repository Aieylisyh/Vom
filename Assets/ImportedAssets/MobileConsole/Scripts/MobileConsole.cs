...
namespace MobileConsole
{
    public class MobileConsole : MonoBehaviour
    {
        ...
        public static MobileConsole instance { get; private set; }

        void Awake()
        {
            if ((disableInReleaseBuild && IsReleaseBuild()) || instance != null)
            {
                instance = null;
                Destroy(gameObject);
            }
        ...
        private void AddListeners()
        {
            ...
            //Debug.Log(openButton.onClick.GetPersistentEventCount());
            Open();
            openButton.onClick.AddListener(Open);
            closeButton.onClick.AddListener(Close);
            clearButton.onClick.AddListener(Clear);
            //Debug.Log(openButton.onClick.GetPersistentEventCount());
            //通过这种方式添加的，就是看不见的onClick.AddListener，editor和log都看不见！
            collapseToggle.onValueChanged.AddListener(ToggleCollapse);
            //Debug.Log(openButton.gameObject + "  openButton " + openButton.gameObject.activeSelf);
        }

        public void ActiveOpenButton()
        {
        ...
        private void Close()
        {
            ...
            openButton.gameObject.SetActive(true);
            openButton.image.raycastTarget = false;
        }

        public void Open()
        {
            //  if (!com.CheatCode.IsEnabled())
            //      return;
