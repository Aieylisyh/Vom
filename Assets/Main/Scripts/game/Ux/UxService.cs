...
namespace game
{
    public class UxService : MonoBehaviour
    {
        ...
        public void AddItem(string id, int amountDelta)
        {
            ...
            var proto = ItemService.instance.GetPrototype(id);
            ...
            var parsedRes = ConfigService.instance.itemConfig.ParseComplexItem(id, amountDelta);
            ...
            if (!finished)
            {
                if (proto != null)
                {
                    allItems.Add(new Item(amountDelta, id));
                }
                else
                {
            ...
            if (proto.usage == ItemPrototype.Usage.Cab)
                CombatAbilityService.instance.RefreshTotalPool();
        ...
        public List<Item> GetInventoryItems()
        {
            ...
            {
                if (!ItemService.instance.GetPrototype(item.id).invHide)
                    res.Add(item);
            }
        ...
        public void SyncLevelPlayedCount(LevelItem crtLevelItem)
        {
            var rpd = GetRawPlayedDays();

            if (crtLevelItem != null)
            {
                var lrpd = crtLevelItem.saveData.lastRawPlayedDays;
                if (lrpd != rpd)
                    crtLevelItem.saveData.lastPlayedCount = 0;
                crtLevelItem.saveData.lastRawPlayedDays = rpd;
            }

            var lrpd_global = gameDataCache.cache.lastRawPlayedDays;
            if (lrpd_global != rpd)
                gameDataCache.cache.lastPlayedCount = 0;
            gameDataCache.cache.lastRawPlayedDays = rpd;
